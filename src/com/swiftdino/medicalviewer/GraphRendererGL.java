...
public class GraphRendererGL implements GLSurfaceView.Renderer {
	...
	public void onDrawFrame(GL10 gl) {
		...
		// Matrix.setLookAtM(mVMatrix, 0, offset.x, offset.y, 1.0f, offset.x,
		// offset.y, 0f, 0f, 1.0f, 0f);
	...
	public void onSurfaceChanged(GL10 gl, int width, int height) {
		...
		if (width > height)
			cOrientation = ORIENTATION_LANDSCAPE;
		else
		...
		if (uiRects[cOrientation][0] == null) {
			uiRects[cOrientation][0] = new Rectangle(new float[] {
					cWidth * .75f, cHeight, 0.0f, cWidth, cHeight, 0.0f,
					cWidth, 0.0f, 0.0f, cWidth * .75f, 0.0f, 0.0f });
		}
	...
	private CGraph[] csv2DataSet() throws IOException {
		...
		for (int i = 0; i < extremeVals.length; i++) {
			if (i % 2 == 0)
				extremeVals[i] = Float.MIN_VALUE;
			else
				extremeVals[i] = Float.MAX_VALUE;
		}
		...
		while (line != null) {
			...
			for (int i = 0; i < sets; i++) {
				...
				if (pt > extremeVals[i * 2])
					extremeVals[i * 2] = pt;
				if (pt < extremeVals[i * 2 + 1])
					extremeVals[i * 2 + 1] = pt;
			}
		...
		for (int i = 0; i < sets; i++) {
			...
			if (i > 0)
				offset = _uiBuffer + _dataSets[i - 1].getMax();
			_dataSets[i] = new CGraph((ArrayList<PointF>) pts[i], i, ranges[i],
					offset, startTime);
		}
	...
	private void loadCSV(int patientID, int fromIndex, int toIndex) {
		...
		try {
			...
			String JSONString = gda.execute(patientID, fromIndex, toIndex)
					.get();
			// Log.d(TAG, "Data Recieved:" + JSONString);
			JSONObject jsonData = new JSONObject(JSONString);

			JSONArray dataArray = (JSONArray) jsonData.get("ecgData");
	...
	public void zoomAll(int width, int height) {
		...
		scale.y = ((float) height)
				/ (_dataSets.length + _uiBuffer * (num + 1.0f));
